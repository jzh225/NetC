        ...
        public static UserLoginData ToUser(string xml)
        {
            ...
            if (!string.IsNullOrEmpty(xml))
            {
                try
                {
                    ...
                    userData.Status = Convert.ToInt32(xmlDoc.SelectSingleNode("user/status").InnerText);

                    //userData.Logins = Convert.ToInt32(xmlDoc.SelectSingleNode("user/logins").InnerText);
